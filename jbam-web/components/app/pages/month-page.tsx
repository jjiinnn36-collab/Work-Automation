...
import { occurrenceCsv, shiftMonth, sortRemaining, toCsv, won } from "@/lib/logic"
...
/** 이번 달 — 원기한이 이 달에 드는 건 (보드와 같은 기준, AC-W6). */
export function MonthPage() {
  ...
  const progress = sortRemaining(data.rows.filter((o) => !o.done))
  const done = data.rows.filter((o) => o.done)
  const overdue = progress.filter((o) => o.severity === "overdue")
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description={`기한이 이 달에 드는 ${data.rows.length}건 · 주말·공휴일로 밀린 건은 실제 납부일을 함께 적습니다`}
        actions={
          <>
            <ButtonGroup>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="진행중" value={data.inProgress} hint={progress.slice(0, 3).map((o) => o.name).join(" · ") || "없음"} />
        <StatCard label="완료" value={data.done} hint={done.slice(0, 3).map((o) => o.name).join(" · ") || "없음"} />
      ...
      <section className="flex flex-col gap-3">
        <h3 className="flex items-baseline gap-2 text-base font-semibold">
          진행중 <span className="text-sm font-normal text-muted-foreground">{progress.length}건</span>
          {overdue.length > 0 && <span className="text-sm text-destructive">기한 지남 {overdue.length}건</span>}
        </h3>
        <OccurrenceTable rows={progress} empty="진행중인 건이 없습니다" />
      </section>

      <section className="flex flex-col gap-3">
        <h3 className="flex items-baseline gap-2 text-base font-semibold">
          완료 <span className="text-sm font-normal text-muted-foreground">{done.length}건</span>
        </h3>
        <OccurrenceTable rows={done} empty="아직 끝난 건이 없습니다" />
      </section>
    </div>
  )
}
